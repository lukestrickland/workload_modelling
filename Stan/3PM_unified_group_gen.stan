...
model {
  ...
  //parameters
  U0 ~ normal(5,5); //centred on midpoint of scale
  phi ~ normal(0,10); //centred on zero
  kappa ~ normal(0,5);
  tau_d ~ normal(0,20); //typically fixed to 4.2  h
  tau_r ~ normal(0,20); //typically fixed to 18.2 h
  tau_la ~ normal(0,20); 
  sigma ~ normal(0,3);
  wc ~ normal(0,5);
  wd ~ normal(-5,10);
...
generated quantities{
  ...
  for(j in 1:Nvalid){
    log_lik[j] = normal_lpdf(fatigue[valid[j]] | S[valid[j]]+W[valid[j]]+kappa*C[valid[j]],sigma);
  }
}
