...
//  tau_la ~ normal(80,20); 
  ...
  // sigma ~ normal(0,3);
   ...
   for(i in 1:Nvalid){
    fatigue[valid[i]] ~ normal(B0 + B_fatigue*(S[valid[i]]+kappa*C[valid[i]]),sigma);
  } 
}
...
generated quantities{
  ...
  for(i in 1:Ntotal){
    pp[i] = normal_rng(B0 +B_fatigue*(S[i]+kappa*C[i]),sigma);
  }
  for(j in 1:Nvalid){
    log_lik[j] = normal_lpdf(fatigue[valid[j]] | B0 + B_fatigue*(S[valid[j]]+kappa*C[valid[j]]),sigma);
  }
}
