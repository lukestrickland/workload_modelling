...
data {
  ...
  vector<lower=0>[Ntotal] workload;
  int<lower=0> Nvalid;
  ...
  // real<lower=0> tau_la;
 //typically not modeled, but necessary for parameter estimation
}  
 
parameters {

  //parameters
  real<lower=-2,upper=1> S0_raw;
...
//  tau_la ~ normal(80,20); 
  ...
  // sigma ~ normal(0,3);
   ...
   for(i in 1:Nvalid){
    fatigue[valid[i]] ~ normal(B0 + B_fatigue*(S[valid[i]]+kappa*C[valid[i]]) + B_workload*workload[valid[i]],sigma);
  } 
}
...
generated quantities{
  ...
  for(i in 1:Ntotal){
    pp[i] = normal_rng(B0 + B_fatigue*(S[i]+kappa*C[i]) + B_workload*workload[i],sigma);
    // Test equivalence with FIPS by removing noise
    //pp[i] = S[i]+kappa*C[i];
  }
  for(j in 1:Nvalid){
    log_lik[j] = normal_lpdf(fatigue[valid[j]] |B_fatigue*(S[valid[j]]+kappa*C[valid[j]])+ B_workload*workload[valid[j]],sigma);
  }
}
