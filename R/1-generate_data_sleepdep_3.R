...
create_sleep_dep_df <- function(wake_datetime,
  ...
  simulation_df <- simulation_df %>% filter(datetime>=sleeptimes$sleep.start[1])
  
  simulation <- FIPS_simulation(
    FIPS_df = simulation_df , # The FIPS_df
    modeltype = "unified",             # three process model
    pvec = unified_make_pvec(L0=  24.12/2, S0 = 24.12/2)      # paramater vector
  )
  ...
  #test theory
  # simulation$fatigue <- lag(simulation$fatigue, 1)
